...
Ver `docs/EXPURGO_ORIGEM.md`.

- **Código / docs (working tree):** 0 ocorrências após a limpeza — comentários
  reescritos para termos neutros, uma chave de `localStorage` renomeada com o
  prefixo `compasso_`, cabeçalhos de `sql/*.sql`, `CLAUDE.md`, `docs/README.md`,
  `MATRIZ_PERMISSOES.md`, e os dois manuais `.docx` regenerados.
- **Pendências binárias:** `docs/DEPARA.xlsx` e
  `docs/CONTROLE DE ACESSO - FUNÇÕES.xlsx` — revisar no Excel.
- **Banco:** `sql/2026-09-03_expurgo_referencias_origem.sql` — Etapa 1
  ...
  `log_expurgo_referencias_origem`. Os padrões de busca/troca vêm de material
  entregue à parte (não versionado).
- **Histórico do Git:** reescrita com `git filter-repo` — *dry-run* sobre
