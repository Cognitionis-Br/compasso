...
async function renderControleOrcamentoView() {
    ...
    if (elVig) elVig.innerText = formatDate(_coPrimeiroDiaMesAtualISO());

    const elLog = document.getElementById('controleOrcamentoLog');
    if (elLog) {
        elLog.innerHTML = cfg.alterado_em
            ? `Modo vigente: <b>${CONTROLE_ORCAMENTO_LABEL[modoAtual]}</b> · última alteração por <b class="uppercase">${escapeHtml(cfg.alterado_por || '-')}</b> em ${formatDateTime(cfg.alterado_em)}${cfg.modo_anterior ? ` (anterior: ${CONTROLE_ORCAMENTO_LABEL[cfg.modo_anterior] || cfg.modo_anterior})` : ''}.`
...
async function _renderHistoricoControleOrcamento() {
    ...
    const fmtDMenos1 = iso => { if (!iso) return '-'; const d = new Date(String(iso).split('T')[0] + 'T00:00:00'); d.setDate(d.getDate() - 1); return formatDate(d); };
